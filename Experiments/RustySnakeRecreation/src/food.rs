...
impl std::fmt::Display for NoPositionError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "No valid space for food to spawn, failed after 999 attempts."
        )
    }
}
pub struct Food {
...
impl Food {
    ...
    pub fn generate_random_cell(rl: &mut RaylibHandle) -> Vector2 {
        let rand_x = rl
            .get_random_value::<i32>(0..CELL_COUNT)
            .clamp(0, CELL_COUNT - 1);
        let rand_y = rl
            .get_random_value::<i32>(0..CELL_COUNT)
            .clamp(0, CELL_COUNT - 1);
    ...
    pub fn generate_random_positon(
        rl: &mut RaylibHandle,
        snake_body: &VecDeque<Vector2>,
    ) -> Result<Vector2, NoPositionError> {
        let mut attempts = 0;
    ...
    pub fn load_texture(rl: &mut RaylibHandle, thread: &RaylibThread) -> Texture2D {
        let image = Image::load_image("src/Graphics/FoodImage.png").expect("Could not load image");
    ...
    pub fn draw(&self, d: &mut RaylibDrawHandle) {
        ...
        d.draw_texture(&self.texture, x as i32, y as i32, Color::WHITE);
    }
}
